...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, patch_size, out_channels, cond_dim=None):
        ...
        cond_dim = cond_dim if cond_dim is not None else hidden_size * 4
    ...
    def __init__(
        ...
        if eta is not None:  # LayerScale Initialization (no layerscale when None)
            self.gamma1 = nn.Parameter(
                eta * torch.ones(hidden_features), requires_grad=True
            )
            self.gamma2 = nn.Parameter(
                eta * torch.ones(out_features), requires_grad=True
            )
        else:
...
def get_3d_sincos_pos_embed_from_grid(embed_dim, grid):
    # Use embed_dim // 3 per axis (each must be even for sin/cos). Total may be < embed_dim;
    # pad to embed_dim so output always matches model's pos_embed shape.
    d = embed_dim // 3
    d = (d // 2) * 2  # ensure even for get_1d_sincos_pos_embed_from_grid
    d = max(d, 2)
    emb_x = get_1d_sincos_pos_embed_from_grid(d, grid[0])  # (X*Y*Z, d)
    emb_y = get_1d_sincos_pos_embed_from_grid(d, grid[1])  # (X*Y*Z, d)
    emb_z = get_1d_sincos_pos_embed_from_grid(d, grid[2])  # (X*Y*Z, d)
    emb = np.concatenate([emb_x, emb_y, emb_z], axis=1)  # (X*Y*Z, 3*d)
    if emb.shape[1] < embed_dim:
        emb = np.concatenate(
            [emb, np.zeros((emb.shape[0], embed_dim - emb.shape[1]), dtype=emb.dtype)],
            axis=1,
        )
    return emb
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = rearrange_many(qkv, "b d (h c) -> b h d c ", h=self.heads)
        out = F.scaled_dot_product_attention(
            q, k, v, scale=self.scale, dropout_p=0.0, is_causal=False
        )
        out = rearrange(
            out, "b h (z x y) c -> b (h c) z x y ", z=z, x=h, y=w
        ).contiguous()
        out = self.to_out(out)
        return out
...
class BiFlowNet(nn.Module):
    def __init__(
        ...
        if self.res_condition:
            self.res_mlp = nn.Sequential(
                nn.Linear(3, time_dim), nn.SiLU(), nn.Linear(time_dim, time_dim)
            )
            time_dim = 2 * time_dim
        cond_dim = time_dim
        # layers
        ...
        self.patch_size = patch_size
        self.x_embedder = PatchEmbed_Voxel(
            sub_volume_size, patch_size, channels, dim, bias=True
        )
        ...
        for i in range(self.feature_fusion):
            ...
            temp.append(FinalLayer(dim, self.patch_size, dim, cond_dim=cond_dim))
        ...
        for i in range(self.num_mid_DiT):
            self.IntraPatchFlow_mid.append(
                DiTBlock(
                    dim,
                    DiT_num_heads,
                    mlp_ratio=mlp_ratio,
                ...
                nn.ModuleList(
                    [
                        ...
                        (
                            Residual(
                                PreNorm(
                                    dim_out, AttentionBlock(dim_out, heads=attn_heads)
                                )
                            )
                            if use_sparse_linear_attn[ind]
                            else nn.Identity()
                        ),
        ...
        self.mid_spatial_attn = Residual(
            PreNorm(mid_dim, AttentionBlock(mid_dim, heads=attn_heads))
        )
                    ...
                    [
                        ...
                        (
                            Residual(
                                PreNorm(
                                    dim_out,
                                    AttentionBlock(dim=dim_out, heads=attn_heads),
                                )
                            )
                            if use_sparse_linear_attn[len(in_out) - ind - 1]
                            else nn.Identity()
                        ),
                        ...
                        (
                            Residual(
                                PreNorm(
                                    dim_in, AttentionBlock(dim=dim_in, heads=attn_heads)
                                )
                            )
                            if use_sparse_linear_attn[len(in_out) - ind - 1]
                            else nn.Identity()
    ...
    def initialize_weights(self):
        ...
        # Zero-init output projection for stable diffusion training.
    ...
    def forward(
        ...
        x_IntraPatch = rearrange(
            x_IntraPatch, "b c p1 p2 p3 d h w -> (b p1 p2 p3) c d h w"
        )
        x = self.init_conv(x)
        r = x.clone()
        ...
        if self.res_condition:
            ...
            res_condition_emb_DiT = (
                res_condition_emb.unsqueeze(1).repeat(1, p1 * p2 * p3, 1).view(-1, c)
            )
        ...
        for Block, MlpLayer in self.IntraPatchFlow_input:
            ...
            Unet_feature = rearrange(
                Unet_feature,
                "b (p1 p2 p3) c d h w -> b c (p1 d) (p2 h) (p3 w)",
                p1=ori_shape[0] // self.vq_size,
                p2=ori_shape[1] // self.vq_size,
                p3=ori_shape[2] // self.vq_size,
            )
        ...
        for idx, (block1, spatial_attn1, block2, spatial_attn2, upsample) in enumerate(
            self.ups
        ):
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def p_sample_loop(self, denoise_fn, z, y=None, res=None, cond_scale=1.0, hint=None):
    ...
    def sample(
        ...
        if strategy == "ddpm":
            return self.p_sample_loop(denoise_fn, z, y, res, cond_scale, hint)
        elif strategy == "ddim":
    ...
    def p_sample_loop_ddim(
        ...
        # DDIM time schedule
        times = (
            torch.linspace(0, self.num_timesteps - 1, steps=ddim_steps)
            .long()
            .flip(0)
            .to(device)
        )
        ...
        for i in tqdm(range(ddim_steps), desc="DDIM Sampling", total=ddim_steps):
            t = times[i]
            t_prev = (
                times[i + 1] if i < ddim_steps - 1 else torch.tensor(0, device=device)
            )
    ...
    def p_sample_ddim(
        ...
        alpha_prev = self.alphas_cumprod[t_prev].view(-1, 1, 1, 1)
        sigma = (
            eta * ((1 - alpha_prev) / (1 - alpha_t) * (1 - alpha_t / alpha_prev)).sqrt()
        )
    ...
    def p_losses(
        self, denoise_fn, x_start, t, y=None, res=None, noise=None, hint=None, **kwargs
    ):
